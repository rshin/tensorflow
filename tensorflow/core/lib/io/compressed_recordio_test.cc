...
class RecordioTest : public ::testing::Test {
 ...
 protected:
  ...
  SnappyWritableFile compressed_dest_;
  SnappyRandomAccessFile compressed_source_;
  bool reading_;
  uint64 readpos_;
 ...
 public:
  RecordioTest()
      : compressed_dest_(&dest_),
        compressed_source_(&source_),
        reading_(false),
        readpos_(0),
        writer_(new RecordWriter(&compressed_dest_)),
        reader_(new RecordReader(&compressed_source_)) {}
...
TEST_F(RecordioTest, CorruptLength) {
  ...
  compressed_dest_.Flush();
  IncrementByte(0, 100);
  AssertHasSubstr(Read(), "Unknown Snappy chunk type");
}
